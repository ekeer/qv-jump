...
// 跳转方式: 极简 HTML + JS location.replace（兼容 QQ/微信内置浏览器）
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api-qq.qun') {
      ...
      return buildRedirect(`mqqapi://card/show_pslcard?src_type=internal&version=1&card_type=group&uin=${qun}`);
    }
    ...
    if (path === '/api-qq.chat') {
      ...
      return buildRedirect(`mqqwpa://im/chat?chat_type=wpa&uin=${qq}&version=1&src_type=web`);
    }
    ...
    if (path === '/api-wx') {
      ...
      return buildRedirect(`weixin://addfriend/${wx}`);
    }
