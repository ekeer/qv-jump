...
// 跳转：外部浏览器 location.replace / QQ微信内 导航页

import { buildRedirect, renderErrorHtml, isValidUin, isValidWechatId, isValidChannelsId } from '../functions/_lib.js';
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api-qq.qun') {
      ...
      return buildRedirect(`mqqapi://card/show_pslcard?src_type=internal&version=1&card_type=group&uin=${qun}`, ua);
    }
    if (path === '/api-qq.chat') {
      const qq = (url.searchParams.get('qq') || '').trim();
      if (!qq) return error('缺少 qq 参数');
      if (!isValidUin(qq)) return error('qq 参数格式不正确（应为 4-14 位纯数字）');
      return buildRedirect(`mqqwpa://im/chat?chat_type=wpa&uin=${qq}&version=1&src_type=web`, ua);
    }
    if (path === '/api-qq') {
      const qq = (url.searchParams.get('qq') || '').trim();
      if (!qq) return error('缺少 qq 参数');
      if (!isValidUin(qq)) return error('qq 参数格式不正确（应为 4-14 位纯数字）');
      return buildRedirect(`mqqapi://card/show_pslcard?src_type=internal&version=1&uin=${qq}`, ua);
    }
    if (path === '/api-wx') {
      ...
      return buildRedirect(`weixin://addfriend/${wx}`, ua);
    }
    if (path === '/api-wx.pay') {
      const payUrl = (env && env.WXP_PAY_URL) || '';
      if (!payUrl) return error('收款码未配置：请在 Cloudflare 后台设置环境变量 WXP_PAY_URL', 500);
      return buildRedirect(payUrl, ua);
    }
    if (path === '/api-wx.scan') {
      return buildRedirect('weixin://dl/scan', ua);
    }
    if (path === '/api-wx.channels') {
      ...
      return buildRedirect(`weixin://channelsprofile?username=${encodeURIComponent(username)}`, ua);
    }
